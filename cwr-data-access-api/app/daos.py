from model.models import Agreement, AgreementType, Territory, AgreementRole, InterestedParty, DistributionCategory, \
    TextMusicRelationship, CompositeType, VersionType, ExcerptType, MusicArrangement, LyricAdaptation, WorkType, Work
...
class AgreementDAO(DAO):

    def __init__(self):
        super(AgreementDAO, self).__init__(Agreement)

    def get_agreements_by_type(self, agreement_type):
        return self.session.query(Agreement).filter(Agreement.agreement_type_id == agreement_type).all()
...
class InterestedPartyDAO(DAO):

    def __init__(self):
        super(InterestedPartyDAO, self).__init__(InterestedParty)
...
class SharesDAO(DAO):
    ...
    def __init__(self):
        super(SharesDAO, self).__init__(SharesDAO)
...
class WorkDAO(DAO):
    ...
    def __init__(self):
        super(WorkDAO, self).__init__(Work)


    def get_by_iswc(self, iswc):

        return self.session.query(self.cls).filter_by(iswc=iswc).first()
